...
#     curl -fsSL https://raw.githubusercontent.com/maelqo/scripts/main/scripts/aiflow/deploy-caddy.sh | bash
#
# Usage (flags -- note the "bash -s --", plain flags after "| bash" are
# silently swallowed since bash reads the script itself from stdin):
#   curl -fsSL https://raw.githubusercontent.com/maelqo/scripts/main/scripts/aiflow/deploy-caddy.sh \
...
# Companion files (docker-compose.caddy.yml, Caddyfile.example) live
# alongside this script under config/, see SCRIPT_REPO_ROOT below.
